...
public Action OnTakeDamageAlive(victim, &attacker, &inflictor, float &damage, &damagetype, &weapon, float damageForce[3], float damagePosition[3], damagecustom){
	...
	if(IsValidClient(victim)){
		...
		if(absorptionAmount[victim]){
			absorptionAmount[victim] -= damage;
			damage = 0.0;
			if(absorptionAmount[victim] < 0.0){
				absorptionAmount[victim] = 0.0;
				TF2_AddCondition(victim, TFCond_SpeedBuffAlly, 4.0);
				TF2_AddCondition(victim, TFCond_DodgeChance, 1.0);
				TF2_AddCondition(victim, TFCond_RadiusHealOnDamage, 1.5);
			}
		}
	}
	return Plugin_Changed;
